'''
Mostly stolen from https://github.com/anthonywritescode/aoc2024
'''
...
def _get_cookie_headers() -> dict[str, str]:
    ...
    return {'Cookie': contents, 'User-Agent': 'ddellspe'}
...
def parse_coords_hash(s: str, *, item: str = '#') -> set[tuple[int, int]]:
    return {
        (x, y)
        for y, line in enumerate(s.splitlines())
        for x, c in enumerate(line)
...
def format_coords_hash(
    ...
    bx, by = bounds(coords)
    return '\n'.join(
        ''.join(item if (x, y) in coords else space for x in bx.range)
        for y in by.range
    )
